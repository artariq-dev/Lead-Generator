...
const VARIATIONS: Record<string, { pattern: PatternVariant; color: string }> = {
  audit: { pattern: "stripes", color: "text-emerald-500" },
  build: { pattern: "memphis", color: "text-blue-500" },
  diagnose: { pattern: "contour", color: "text-purple-500" },
};
...
export function HeroCard({
  ...
  const variation = VARIATIONS[iconId] ?? VARIATIONS.audit;
  return (
    <Link
      href={href}
    ...
    >
      <AbstractPattern
        ...
        className={`${variation.color} transition-opacity duration-300 opacity-80 group-hover:opacity-0`}
      />
